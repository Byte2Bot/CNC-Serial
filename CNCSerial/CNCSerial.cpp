...
void CNCSerial::setPaused(int value)
{
	pPaused = 0;
	if(value)
		pPaused = 1;
...
int CNCSerial::buttonPollingThread()
{
	...
	try
	{
		...
		while(!pDone)
		{
			...
			{
				//active edge!  toggle the pPaused flag
				pPaused++;
				pPaused &= 0x01;
				
				log("Start/Pause button pressed!  pPaused=%d\n", pPaused);
...
int CNCSerial::serialPollingThread()
{
	...
	try
	{
		...
		while(!pDone)
		{
			...
			if(pSendingFile && pClearToSend && (!pPaused))  //if we are sending a file and not held up by flow control
			{
				//if(!file_fd)
				//{
			...
			if(pClearToSend && (!pPaused) && !txData.isEmpty() && delay_timer.Expired(pPacketDelay))
			{
